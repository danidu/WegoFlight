...
@Service
public class AmadeusFlightProvider implements FlightAggregatorProvider {
    
    private static final Logger logger = LoggerFactory.getLogger(AmadeusFlightProvider.class);
    
    private final WebClient webClient;
    
    @Value("${aggregator.amadeus.base-url:https://api.amadeus.com}")
    private String baseUrl;
    
    @Value("${aggregator.amadeus.timeout-seconds:10}")
    private int timeoutSeconds;
    
    public AmadeusFlightProvider(WebClient.Builder webClientBuilder, 
                                 @Value("${aggregator.amadeus.base-url:https://api.amadeus.com}") String baseUrl) {
    ...
    @Cacheable(value = "flightSearchCache", key = "#request.origin + '-' + #request.destination + '-' + #request.departureDate + '-amadeus'")
    public List<FlightSearchResponse.FlightOption> searchFlights(FlightSearchRequest request) {
        // Log request
        logger.info("[AMADEUS] Request received - Origin: {}, Destination: {}, DepartureDate: {}, ReturnDate: {}, Adults: {}, Children: {}, Infants: {}, CabinClass: {}, DirectFlightsOnly: {}, MaxStops: {}, PreferredAirlines: {}, MaxResults: {}",
            request.getOrigin(),
        ...
        List<FlightSearchResponse.FlightOption> results = generateDummyFlights(request, "Amadeus");
        long duration = System.currentTimeMillis() - startTime;
        
        // Log response
        logger.info("[AMADEUS] Response sent - Status: SUCCESS, FlightOptionsCount: {}, Duration: {}ms, TotalPriceRange: ${}-${}",
            results.size(),
            duration,
    ...
    public List<FlightSearchResponse.FlightOption> retryFallbackSearch(FlightSearchRequest request, Exception e) {
        logger.warn("[AMADEUS] Retry exhausted - Request: Origin={}, Destination={}, DepartureDate={}, Attempts: 3, Error: {}", 
        ...
        throw new RuntimeException("Amadeus API retry exhausted: " + e.getMessage(), e);
    }
    ...
    public List<FlightSearchResponse.FlightOption> fallbackSearch(FlightSearchRequest request, Exception e) {
        logger.warn("[AMADEUS] Circuit breaker opened - Request: Origin={}, Destination={}, DepartureDate={}, Error: {}", 
            request.getOrigin(), request.getDestination(), request.getDepartureDate(), e.getMessage());
        logger.warn("[AMADEUS] Response sent - Status: FAILED (Circuit Breaker), FlightOptionsCount: 0, Error: {}", e.getMessage());
    ...
    private List<FlightSearchResponse.FlightOption> generateDummyFlights(FlightSearchRequest request, String provider) {
        ...
        String[] carriers = {"EK", "QR", "SQ", "LH", "BA"};
        String[] aircraftTypes = {"Boeing 777", "Airbus A350", "Boeing 787", "Airbus A380"};
        ...
        for (int i = 0; i < 4; i++) {
            ...
            LocalDateTime departure = request.getDepartureDate().atTime(6 + i * 3, 0);
            LocalDateTime arrival = departure.plusHours(8 + i).plusMinutes(30);
            
            FlightSearchResponse.FlightSegment segment = new FlightSearchResponse.FlightSegment();
            segment.setCarrier(carrier);
            segment.setFlightNumber(carrier + (100 + i * 10));
            ...
            if (request.getReturnDate() != null) {
                LocalDateTime returnDeparture = request.getReturnDate().atTime(14 + i, 0);
                LocalDateTime returnArrival = returnDeparture.plusHours(9 + i).plusMinutes(15);
                
                FlightSearchResponse.FlightSegment returnSegment = new FlightSearchResponse.FlightSegment();
                returnSegment.setCarrier(carrier);
                returnSegment.setFlightNumber(carrier + (200 + i * 10));
            ...
            breakdown.setChildren((basePrice * 0.75) * (request.getChildren() != null ? request.getChildren() : 0));
